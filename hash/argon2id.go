...
var (
	...
	ArgonErrInvalidHash = errors.New("argon2id: hash is not in the correct format")
	...
	ArgonErrIncompatibleVariant = errors.New("argon2id: incompatible variant of argon2")
	...
	ArgonErrIncompatibleVersion = errors.New("argon2id: incompatible version of argon2")
)
...
var ArgonDefaultParams = &ArgonParams{
	Memory:      64 * 1024,
	Iterations:  1,
...
func ArgonCreateHash(password string, params *ArgonParams) (hash string, err error) {
	salt, err := argonGenerateRandomBytes(params.SaltLength)
	if err != nil {
		return "", err
	}
...
func ArgonCheckHash(password, hash string) (match bool, params *ArgonParams, err error) {
	params, salt, key, err := ArgonDecodeHash(hash)
	if err != nil {
...
func ArgonDecodeHash(hash string) (params *ArgonParams, salt, key []byte, err error) {
	...
	if len(vals) != 6 {
		return nil, nil, nil, ArgonErrInvalidHash
	}

	if vals[1] != "argon2id" {
		return nil, nil, nil, ArgonErrIncompatibleVariant
	}
	...
	if version != argon2.Version {
		return nil, nil, nil, ArgonErrIncompatibleVersion
	}
